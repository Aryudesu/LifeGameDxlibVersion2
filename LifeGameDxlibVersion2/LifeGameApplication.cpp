...
void LifeGameApplication::update(const InputFrame& input, double elapsedSeconds) {
    ...
    if (input.mouseX < AppConfig::BoardViewWidth) {
        if (input.mouseWheel > 0) {
            for (int i = 0; i < input.mouseWheel; ++i) {
                if (!camera_.zoomInAt(input.mouseX, input.mouseY, AppConfig::MaxCellSize)) break;
            }
        } else if (input.mouseWheel < 0) {
            for (int i = 0; i < -input.mouseWheel; ++i) {
                if (!camera_.zoomOutAt(input.mouseX, input.mouseY, AppConfig::MinCellSize)) break;
            }
        }
    }
    ...
    if (input.mouseMiddleDown && input.mouseX < AppConfig::BoardViewWidth) {
...
void LifeGameApplication::drawToolPanel() const {
    ...
    DrawBox(PanelContentX, 42, AppConfig::WindowWidth - PanelPadding, 72,
        selectedPatternIndex_ == 0 ? selected : section, TRUE);
    DrawString(PanelContentX + 10, 49, "Cell", text);

    const PatternCategory categories[] = {
        PatternCategory::StillLife, PatternCategory::Oscillator,
        PatternCategory::Spaceship, PatternCategory::Gun
    };
    for (int i = 0; i < 4; ++i) {
        ...
        DrawBox(left, top, left + 136, top + 28,
            toolCategory_ == categories[i] ? selected : section, TRUE);
    ...
    for (std::size_t i = 1; i < PatternLibrary::size(); ++i) {
        ...
        DrawBox(PanelContentX, top, AppConfig::WindowWidth - PanelPadding, top + 24,
            selectedPatternIndex_ == i ? selected : section, TRUE);
    ...
    DrawFormatString(PanelContentX, infoY + 122, text, "Alive       %llu (%.2f%%)",
        static_cast<unsigned long long>(aliveCells), alivePercentage);
    ...
    DrawString(PanelContentX, infoY + 202, paused_ ? "PAUSED" : "RUNNING",
        paused_ ? GetColor(255, 210, 90) : GetColor(120, 230, 140));
...
void LifeGameApplication::setPaused(bool paused) {
    paused_ = paused;
    simulationAccumulator_ = 0.0;
    SetMainWindowText(paused_ ? AppConfig::PausedWindowTitle : AppConfig::WindowTitle);
}

void LifeGameApplication::saveBoard() {
    std::string path;
    if (!FileDialog::chooseSavePath(path)) return;
    std::string errorMessage;
    if (!LifeFile::save(board_, generation_, path, errorMessage)) FileDialog::showError(errorMessage);
}

void LifeGameApplication::loadBoard() {
    std::string path;
    if (!FileDialog::chooseLoadPath(path)) return;
    std::string errorMessage;
...
void LifeGameApplication::editBoardWithMouse(const InputFrame& input) {
    if (input.mouseX < 0 || input.mouseY < 0 ||
        input.mouseX >= AppConfig::BoardViewWidth || input.mouseY >= AppConfig::ScreenHeight) return;

    const auto [boardX, boardY] = camera_.screenToBoard(input.mouseX, input.mouseY);
    if (selectedPatternIndex_ == 0) {
        if (input.mouseLeftDown) board_.setAlive(boardX, boardY, true);
...
void LifeGameApplication::handleToolPanel(const InputFrame& input) {
    ...
    if (inRect(input.mouseX, input.mouseY, PanelContentX, 42,
        AppConfig::WindowWidth - PanelPadding, 72)) {
        selectPattern(0);
        return;
    }
    ...
    for (std::size_t i = 1; i < PatternLibrary::size(); ++i) {
        ...
        if (pattern.category != toolCategory_) continue;
        const int top = PatternListY + row * PatternRowHeight;
        if (inRect(input.mouseX, input.mouseY, PanelContentX, top,
            AppConfig::WindowWidth - PanelPadding, top + 24)) {
            selectPattern(i);
            return;
        }
        ++row;
...
void LifeGameApplication::placeSelectedPattern(int boardX, int boardY) {
    const LifePattern& pattern = currentPattern();
    for (const PatternCell cell : pattern.cells) {
        const PatternCell rotated = rotateCellClockwise(cell, patternRotationQuarterTurns_);
        board_.setAlive(boardX + rotated.x, boardY + rotated.y, true);
    }
}

void LifeGameApplication::selectNextPattern() noexcept {
    selectPattern((selectedPatternIndex_ + 1) % PatternLibrary::size());
}

void LifeGameApplication::selectPreviousPattern() noexcept {
    selectPattern((selectedPatternIndex_ + PatternLibrary::size() - 1) % PatternLibrary::size());
}

void LifeGameApplication::selectPattern(std::size_t index) noexcept {
    selectedPatternIndex_ = index % PatternLibrary::size();
    patternRotationQuarterTurns_ = 0;
    const PatternCategory category = currentPattern().category;
    if (category != PatternCategory::Cell) toolCategory_ = category;
}

void LifeGameApplication::rotatePattern(int direction) noexcept {
    if (selectedPatternIndex_ == 0 || direction == 0) return;
    patternRotationQuarterTurns_ =
        (patternRotationQuarterTurns_ + (direction > 0 ? 1 : 3)) % 4;
}

const LifePattern& LifeGameApplication::currentPattern() const noexcept {
    return PatternLibrary::at(selectedPatternIndex_);
}
...
void LifeGameApplication::changeSimulationSpeed(int direction) {
    if (direction > 0) {
        if (simulationSpeedIndex_ + 1 < AppConfig::SimulationSpeeds.size()) ++simulationSpeedIndex_;
