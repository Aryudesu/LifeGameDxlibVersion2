...
PatternCell rotateCellClockwise(PatternCell cell, int quarterTurns) {
    int x = cell.x;
    int y = cell.y;
    for (int i = 0; i < quarterTurns; ++i) { const int nextX = -y; const int nextY = x; x = nextX; y = nextY; }
    return {x, y};
}
...
int LifeGameApplication::run() {
    using Clock = std::chrono::steady_clock; const auto targetFrameDuration = std::chrono::duration_cast<Clock::duration>(std::chrono::duration<double>(1.0 / AppConfig::TargetFps));
    auto previousFrameTime = Clock::now(); auto nextFrameTime = previousFrameTime; auto fpsSampleStart = previousFrameTime; int fpsFrameCount = 0;
...
void LifeGameApplication::update(const InputFrame& input, double elapsedSeconds) {
    handleToolPanel(input); if (input.togglePause) setPaused(!paused_);
    ...
    camera_.move(input.cameraDeltaX * AppConfig::CameraMoveSpeed, input.cameraDeltaY * AppConfig::CameraMoveSpeed); if (input.mouseMiddleDown && input.mouseX < AppConfig::BoardViewWidth) camera_.panByPixels(input.mouseDeltaX, input.mouseDeltaY); else camera_.endPan();
    if (paused_) { simulationAccumulator_ = 0.0; editBoardWithMouse(input); if (input.singleStep) advanceGeneration(); return; }
    const double simulationSeconds = std::min(elapsedSeconds, AppConfig::MaxSimulationDeltaSeconds); simulationAccumulator_ += simulationSeconds * currentSimulationSpeed(); const int generationsToAdvance = static_cast<int>(simulationAccumulator_); simulationAccumulator_ -= generationsToAdvance; for (int i = 0; i < generationsToAdvance; ++i) advanceGeneration();
}
...
void LifeGameApplication::handleToolPanel(const InputFrame& input) {
    if (input.mouseX < PanelX) return; if (inRect(input.mouseX, input.mouseY, PanelContentX, PatternListY, AppConfig::WindowWidth - PanelPadding, PatternListBottom)) patternListScroll_.scroll(toolCategory_, input.mouseWheel); if (!input.mouseLeftPressed) return;
